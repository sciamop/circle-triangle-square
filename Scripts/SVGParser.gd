...
SVG PARSER
"""
var file_path = "res://Sprites/grappling-hook-full.svg"
...
func _run() -> void:
	...
	#clear tree
	if root_node and root_node.get_children():
...
func process_svg_path(element:XMLParser) -> void:
	...
	var string_arrays = []
	var big_fat_string_array: PackedStringArray
	for a in element_string_array:
		if a == "m" or a == "M":
			if big_fat_string_array.size() > 0:
				string_arrays.append(big_fat_string_array)
				big_fat_string_array.resize(0)
		big_fat_string_array.append(a)
	string_arrays.append(big_fat_string_array)
